...
class TrackingImageView: UIView, AVSpeechSynthesizerDelegate {
    ...
    var rectToPlate = [UUID: String]()
    var plateDetectRes: NSDictionary!
    ...
    override func draw(_ rect: CGRect) {
        ...
        for polyRect in self.polyRects {
            ...
            isGoal = (self.rectToPlate[uuid] != nil && self.rectToPlate[uuid] == goalCar)
...
//            print(self.trackDetectUUID)
            ...
            for item in self.rectToPlate {
                if !self.trackDetectUUID.isEmpty && self.trackDetectUUID.keys.contains(uuid) {
                    if item.key == self.trackDetectUUID[uuid] {
                        let val = item.value
                        self.rectToPlate.removeValue(forKey: item.key)
                        self.rectToPlate[uuid] = val
                    }
                }
            }
...
//            print(isGoal)
            ...
            if isGoal {
                fillColor = polyRect.color.cgColor.copy(alpha: 0.5)
            }
            else {
                fillColor = polyRect.color.cgColor.copy(alpha: 0.2)
            }
...
//            print(target_image.size)
            ...
            if cnt > 4 && pointsEqual(point1: centerPoints[cnt-1], point2: centerPoints[cnt-2], point3: centerPoints[cnt-3], point4: centerPoints[cnt-4]) && isGoal {
                print("车到了～～～")
                if canRead {
                    let rectCenterX = topLeft_point.x + target_Size.width / 2
    //                let rectCenterY = topLeft_point.y + target_Size.height / 2
                    let walkVitual = self.imageAreaRect.width / 2 - rectCenterX
    //                真实小轿车宽度为1.8m
                    let carWidthActual = 1.8
                    let temp = CGFloat(carWidthActual) * walkVitual / target_Size.width
                    var walkActual = roundTo(places: 1, num: temp.native)
                    let direction = walkActual > 0 ? "右": "左"
                    walkActual = abs(walkActual)
                    let str = "请先向" + direction + "走\(walkActual)米，在一直向前走"
            ...
            let scalePlateX: CGFloat = self.imageAreaRect.width/self.videoWidth
            let scalePlateY: CGFloat = self.imageAreaRect.height/self.videoHeight
            
//            print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~plate~~~~~~~~~~~~~~~~~~~~~~~")
//            print(self.plateDetectRes)
            if self.plateDetectRes != nil {
                for plate in self.plateDetectRes {
                    let oneDict = plate.value as! NSDictionary
                    let plateName = oneDict["Res"] as! String
                    let plateCX = oneDict[plateName + "CX"] as! String
                    let plateCY = oneDict[plateName + "CY"] as! String
                    let cgCX = StringToFloat(str: plateCX)
                    let cgCY = StringToFloat(str: plateCY)
                    let inRect = isInRect(cx: cgCX * scalePlateX, cy: cgCY * scalePlateY, rectX: topLeft_point.x, rectY: topLeft_point.y - 230.0, rectW: target_Size.width, rectH: target_Size.height)
                    if(inRect) {
                        if(canChange) {
                            self.rectToPlate[polyRect.observation.uuid] = plateName
                            if(plateName == self.goalCar) {
                                canChange = false
                            }
                        }
                    }
            ...
            // 获取显示目标标签(画文本)
//            let str = polyRect.observation.uuid
            var plateStr = polyRect.label
//            print("!!!!!!!!!!!!!!!!!!!!!!!!")
////            print(str)
//            print(self.rectToPlate)
//            plateStr = getLabel(centers: centers, polyRect: polyRect)
            if self.rectToPlate[uuid] != nil {
                plateStr = self.rectToPlate[uuid]!
            }
            let attrString = NSAttributedString(string: plateStr, attributes: [NSAttributedString.Key.font:UIFont.systemFont(ofSize: 11, weight: .regular), NSAttributedString.Key.backgroundColor:polyRect.color])
            attrString.draw(at: previous_leftTop)
            
            // 显示目标番号
            var object_count = String(polyRect.count)
            if self.rectToPlate[uuid] != nil && self.rectToPlate[uuid] == goalCar {
                object_count = "目标车辆"
            }
    ...
    func roundTo(places:Int, num: Double) -> Double {
    ...
    func pointsEqual(point1: CGPoint, point2: CGPoint, point3: CGPoint, point4:CGPoint) -> Bool{
        let ok1 =  abs(point1.x-point2.x) < 0.01 && abs(point1.y - point2.y) < 0.01 ? true : false
        let ok2 =  abs(point2.x-point3.x) < 0.01 && abs(point2.y - point3.y) < 0.01 ? true : false
        let ok3 =  abs(point3.x-point4.x) < 0.01 && abs(point3.y - point4.y) < 0.01 ? true : false
        let res = ok1 && ok2 && ok3
        return res
    }
    
    func getLabel(centers: [Center_UUID], polyRect: TrackedPolyRect) -> String {
        var plateStr = polyRect.label
//        if self.rectToPlate[]
//        if centers.count > 1{
//            for center in centers {
//                if self.rectToPlate[center.uuid] != nil {
//                    plateStr = self.rectToPlate[center.uuid]!
//                    return plateStr;
//                }
//            }
//        }
    ...
    func isInRect(cx: CGFloat, cy: CGFloat, rectX: CGFloat, rectY: CGFloat, rectW: CGFloat, rectH: CGFloat) -> Bool {

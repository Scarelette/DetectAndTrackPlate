...
//                    print(" o（^_^）o ・・>>>>>>>>>>>>>>>>>>>>> 进入检测进程：")
                ...
                let res = Utility.detect(image)
                self.plateDetectRes = res as NSDictionary
                if res != nil {
                    print(res)
                }
            }
...
//                        inRect = isInRect(cx: cgCX, cy: cgCY, rect: rect)
//                        if(inRect) {
//                            rectToPlate[rect.observation.uuid] = plateName
//                        }
//                    }
                }
            }
...
//            print("~~~~~~~~~~~~~~~~~~~~~~~~ 追踪目标全消失 ~~~~~~~~~~~~~~~~~~~~~")
    ...
    func isInRect(cx: CGFloat, cy: CGFloat, rect: TrackedPolyRect) -> Bool {
        if(cx > rect.bottomLeft.x && cx < rect.bottomRight.x && cy > rect.topLeft.y && cy < rect.bottomLeft.y) {
            return true
        }
        return false
    }
